...
where:
  ρ    : scalar correlation factor (learned from data)
...
class MultiFidelityGP:
    ...
    Training procedure:
      ...
      2. Compute ρ = cov(HF, LF_predictions) / var(LF_predictions)
      3. Compute HF residuals: r_i = y_HF_i - ρ·GP_lf.predict(x_HF_i)
      4. Fit GP_delta on (x_HF, r) residuals

    Prediction:
      ŷ_HF(x) = ρ·GP_lf(x) + GP_delta(x)
    """
    ...
    def fit(
        ...
        # ρ = least-squares scaling factor
        # min ||y_hf - ρ·lf_pred||² → ρ = (lf·hf) / (lf·lf)
        self._rho = float(
            np.dot(lf_pred_at_hf, y_hf) /
            (np.dot(lf_pred_at_hf, lf_pred_at_hf) + 1e-12)
        )
        log.info("  Estimated ρ (LF–HF correlation): %.4f", self._rho)
